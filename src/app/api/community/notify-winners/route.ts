  ...
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      <p style="color:#333;font-size:16px;line-height:1.6;">
        Ai terminat luna aceasta pe locul <strong>${params.rank}</strong> cu <strong>${params.points} puncte</strong>. Ca recunoaștere, câștigați:
      </p>
      ...
      <a href="${params.bookingUrl}" style="display:block;background:#f5c842;color:#1a2f4e;text-decoration:none;text-align:center;font-weight:700;font-size:16px;padding:14px 24px;border-radius:10px;margin:24px 0;">
        Rezervă-ți ședința →
      </a>
    ...
    <div style="background:#f5f5f5;padding:16px;text-align:center;">
      <p style="color:#aaa;font-size:12px;margin:0;">Comunitate pe <a href="https://docourse.ro" style="color:#f5c842;">DoCourse</a></p>
    </div>
  </div>
</body>
</html>`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!supabaseUrl || !serviceKey) {
      return NextResponse.json({ error: "SUPABASE config lipsă" }, { status: 500 });
    }
    if (!resendKey) {
      return NextResponse.json({ error: "RESEND_API_KEY lipsă din Vercel env" }, { status: 500 });
    }
    ...
    try {
      ...
      const msg = err instanceof z.ZodError ? err.issues[0]?.message ?? "Date invalide" : "Date invalide";
      return NextResponse.json({ error: msg }, { status: 400 });
    }

    const { communityId, bookingUrl, communityName, rewardText } = body;

    // Use admin client with user's JWT for creator verification
    ...
    // Verify caller is the community creator
    ...
    if (winnersError) {
      return NextResponse.json({ error: `DB error: ${winnersError.message}` }, { status: 500 });
    }
    if (!winners || (winners as WinnerRow[]).length === 0) {
    ...
    for (let i = 0; i < (winners as WinnerRow[]).length; i++) {
      ...
      if (res.ok) { sent.push(winner.full_name); } else { failed.push(winner.full_name); }
    }
