                ...
                <li key={item.label}>
                  <Link href={item.href}
                    className={cn(
                      ...
                      item.active ? "bg-gold/10 text-gold font-medium" : "text-charcoal hover:bg-beige hover:text-navy"
                    )}
                  >
          ...
          <div className="lg:hidden fixed inset-0 z-50">
            ...
            <aside className="absolute left-0 top-0 bottom-0 w-64 bg-background animate-slide-in-left">
              <div className="p-6 border-b border-border flex items-center justify-between">
                <Logo size="sm" />
                <button onClick={() => setSidebarOpen(false)}><X className="w-6 h-6 text-charcoal" /></button>
              </div>
              <nav className="p-4">
                <ul className="space-y-1">
                  {menuItems.map((item) => (
